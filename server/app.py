# server/app.py
import os, json, shutil, datetime, base64, zipfile, io, csv, re, time, secrets
...
# ======== Config ========
# Never ship a hardcoded JWT secret: a known signing key lets anyone forge tokens.
# Require SECRET in production; otherwise generate an ephemeral one (tokens reset on restart).
SECRET = os.getenv("SECRET")
if not SECRET:
    SECRET = secrets.token_urlsafe(64)
    print("WARNING: SECRET env var not set — generated an ephemeral secret. "
          "Set SECRET to keep tokens valid across restarts.")
ALGO = "HS256"
ACCESS_MINUTES = 60 * 8

# Upload limits (bytes) to bound memory use and resist DoS / zip bombs.
MAX_UPLOAD_BYTES = int(os.getenv("MAX_UPLOAD_BYTES", str(25 * 1024 * 1024)))
MAX_ZIP_UNCOMPRESSED_BYTES = int(os.getenv("MAX_ZIP_UNCOMPRESSED_BYTES", str(50 * 1024 * 1024)))
...
# Pin CORS to known origins. "*" with credentials is invalid and over-permissive.
ALLOWED_ORIGINS = [
...
def get_token(
    ...
    """Prefer the Authorization: Bearer header; fall back to ?token= for
    browser-driven downloads (window.open) that cannot set headers."""
...
def read_upload(file: UploadFile, max_bytes: int = MAX_UPLOAD_BYTES) -> bytes:
    """Read an upload into memory with a hard size cap."""
    data = file.file.read(max_bytes + 1)
    if len(data) > max_bytes:
        raise HTTPException(413, f"File exceeds maximum size of {max_bytes} bytes")
    return data
...
class RegisterIn(BaseModel):
    email: EmailStr
    name: str
    role: Optional[str] = None   # accepted for compatibility but ignored server-side
    password: str
...
def register(inp: RegisterIn, db: Session = Depends(get_db)):
    ...
    # Never trust a client-supplied role: self-registration is always a student.
    # Professors are provisioned by seeding / an admin process, not this endpoint.
    user = User(email=inp.email, name=inp.name, role="student", password_hash=pwdctx.hash(inp.password))
...
def upload_assignment(
    ...
    # 4) safe filename + path (sanitized to prevent path traversal)
    original_name = safe_filename(file.filename, default="template.pdf")
    ...
    # 5) read (size-capped) and write file to disk
    try:
    ...
    try:
        template_hash = sha256_bytes(file_bytes)
    except Exception as e:
...
def prof_assignments_all(token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
...
def issue_assignment(inp: IssueIn, token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor": raise HTTPException(403, "professor only")
    ...
    for sid in inp.student_ids:
        ...
        iss = Issuance(
            ...
            tag_b64=b64e(tag),
            aes_key_b64=wrap_key(key),   # wrapped with master key, not stored in cleartext
            status="ISSUED"
        )
...
def prof_list_issuances_all(token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
...
def prof_list_issuances_all_csv(token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
...
def prof_list_submissions_all(token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
...
def prof_list_submissions_all_csv(token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
...
def student_issued(token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
...
def download_encrypted(issuance_id: int, token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
...
def decrypt_and_stream(issuance_id: int, token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    i = db.query(Issuance).get(issuance_id)
    if not i or (me.role=="student" and i.student_id != me.id):
        raise HTTPException(404, "not found")
    ...
    # advance status if not yet submitted
    if i.status in ("ISSUED", "DOWNLOADED"):
        i.status = "DECRYPTED"
        db.commit()

    # Stream from memory — do not persist decrypted plaintext to disk.
    filename = f"issuance_{i.id}_decrypted.pdf"
    return StreamingResponse(io.BytesIO(pt), media_type="application/pdf", headers={
        "Content-Disposition": f'attachment; filename="{filename}"',
...
def view_decrypted_inline(issuance_id: int, token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    i = db.query(Issuance).get(issuance_id)
    if not i or (me.role == "student" and i.student_id != me.id):
        raise HTTPException(404, "not found")
...
def decrypt_from_upload(token: str = Depends(get_token), file: UploadFile = File(...), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "student":
        raise HTTPException(403, "student only")

    # 1️⃣ Read uploaded ZIP into memory (size-capped)
    data = read_upload(file)
    try:
        zbuf = io.BytesIO(data)
        with zipfile.ZipFile(zbuf, "r") as z:
            # Guard against zip bombs: bound total uncompressed size.
            total = sum(zi.file_size for zi in z.infolist())
            if total > MAX_ZIP_UNCOMPRESSED_BYTES:
                raise HTTPException(413, "Package contents too large")
    ...
    key = unwrap_key(target.aes_key_b64)
    try:
...
def submit_answer(issuance_id: int, token: str = Depends(get_token), file: UploadFile = File(...), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "student": raise HTTPException(403, "student only")
    i = db.query(Issuance).get(issuance_id)
    ...
    # save submission — sanitize the client filename to prevent path traversal
    data = read_upload(file)
    ts = int(datetime.datetime.now(datetime.timezone.utc).timestamp())
    fname = safe_filename(file.filename, default="answer.pdf")
    spath = os.path.join(STORE, "submissions", f"iss{i.id}_{ts}_{fname}")
    with open(spath, "wb") as f:
        f.write(data)

    # compute answerHash (hash of raw bytes for demo; your thesis uses answers/ region)
    answer_hash = sha256_bytes(data)
...
def prof_submissions(assignment_id: int, token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor": raise HTTPException(403, "professor only")
...
def prof_list_issuances(assignment_id: int, token: str = Depends(get_token), db: Session = Depends(get_db)):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
...
def _require_professor(db: Session, token: str):
    me = jwt_current(db, token)
    if me.role != "professor":
        raise HTTPException(403, "professor only")
    return me
...
def demo_sign_header(header_json: str = Form(...), token: str = Depends(get_token), db: Session = Depends(get_db)):
    # Uses the real signing key — restrict so it can't be used to forge headers.
    _require_professor(db, token)
...
def demo_aes_gcm_encrypt(header_json: str = Form(...), file: UploadFile = File(...), token: str = Depends(get_token), db: Session = Depends(get_db)):
    # Uses the real signing key — restrict to professors.
    _require_professor(db, token)
    aad = _canon_from_text(header_json)

    # read file (size-capped)
    data = read_upload(file)
    key = os.urandom(32)
    iv  = os.urandom(12)
